...
func _build_road_grid() -> void:
	...
	_add_node("blvd_w",    Vector2(-540,   0))
	_add_node("blvd_docks",Vector2(-420,   0))
	_add_node("blvd_dt_w", Vector2(-180,   0))
	_add_node("blvd_dt_c", Vector2(   0,   0))
	_add_node("blvd_dt_e", Vector2( 180,   0))
	_add_node("blvd_ship", Vector2( 360,   0))
	_add_node("blvd_e",    Vector2( 540,   0))

	# --- Side street intersections ---
	_add_node("dock_n",  Vector2(-420, -180))
	_add_node("dock_s",  Vector2(-420,  180))
	_add_node("dt_w_n",  Vector2(-180, -180))
	_add_node("dt_w_s",  Vector2(-180,  180))
	_add_node("dt_e_n",  Vector2( 180, -180))
	_add_node("dt_e_s",  Vector2( 180,  180))
	_add_node("ship_n",  Vector2( 360, -180))
	_add_node("ship_s",  Vector2( 360,  180))

	# --- Building entrance nodes ---
	_add_node("intake",         Vector2(-480, -120))
	_add_node("dispatcher",     Vector2(-480,  120))
	_add_node("research",       Vector2(-120, -120))
	_add_node("code",           Vector2( 120, -120))
	_add_node("review",         Vector2(-120,  120))
	_add_node("supervisor",     Vector2( 120,  120))
	_add_node("output-dock",    Vector2( 420, -120))
	_add_node("error-chamber",  Vector2( 420,  120))
